...
Much of the core API is built around an idiomatic asynchronous
event-driven architecture in which certain kinds of objects (called "emitters")
periodically emit named events that cause Function objects ("listeners") to be
called.

For instance: a [`fs.ReadStream`][] emits an event when the file is opened;
...
uses the traditional style prototypical inheritance using
...
stack trace is printed, and the app process exits.
...
To guard against crashing the app, developers can either register
